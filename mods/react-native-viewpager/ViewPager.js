...
var ViewPager = React.createClass({
  ...
  getInitialState() {
    return {
      //HACK: new currentPage propert to rewind rules to first screen
        //currentPage: 0,
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.dataSource) {
      var maxPage = nextProps.dataSource.getPageCount() - 1;
        //HACK: new currentPage property to rewind rules to first screen
        //var constrainedPage = Math.max(0, Math.min(this.state.currentPage, maxPage));
        var constrainedPage = Math.max(0, Math.min(this.props.currentPage, maxPage));
      this.setState({
